...
def plot_line_update_webgl(viz, env, args):
    ...
    viz.line(
        ...
        env=env,
        opts=dict(
            title="{} points using WebGL".format(webgl_num_points * 2), webgl=True
        ),
    )
...
def plot_line_many_updates(viz, env, args):
    ...
    for i in range(1, 101):
        ...
        viz.line(
            ...
            Y=np.column_stack(
                (offset1 + np.linspace(5, 10, 10), offset2 + np.linspace(5, 10, 10))
            ),
            env=env,
            win=win,
            update="append",
...
def plot_line_pytorch(viz, env, args):
    try:
        ...
        viz.line(Y=torch.Tensor([[0.0, 0.0], [1.0, 1.0]]), env=env)
    except ImportError:
