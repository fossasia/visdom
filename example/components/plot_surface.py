...
def plot_surface_basic(viz, env, withnames=False):
    columnnames = (
        ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"] if withnames else None
    )
...
def plot_surface_append(viz, env, withnames=False):
    ...
    viz.heatmap(
        ...
        update="appendColumn",
        opts=dict(
            columnnames=["c1", "c2", "c3"] if withnames else None, colormap="Rainbow"
        ),
        env=env,
    )
    viz.heatmap(
...
def plot_surface_remove(viz, env, withnames=False):
    ...
    win = viz.heatmap(X=None, win=win, update="remove", env=env)
