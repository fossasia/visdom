...
def _pc3d_validate_xyz(xyz):
    ...
    if xyz.ndim != 2 or xyz.shape[1] != 3:
        raise ValueError("xyz must have shape (N, 3); got {}".format(xyz.shape))
    if xyz.shape[0] == 0:
...
def _pc3d_validate_rgb(rgb, N):
    ...
    if rgb.ndim != 2 or rgb.shape != (N, 3):
        raise ValueError("rgb must have shape ({}, 3); got {}".format(N, rgb.shape))
    ...
    return np.ascontiguousarray(np.round(rgb_f).clip(0, 255).astype(np.uint8))
...
def _pc3d_downsample(xyz, rgb, opts):
    ...
    mode = (
        opts.get("downsample")
        if opts.get("downsample") in ("stride", "random")
        else "stride"
    )
    if mode == "stride":
