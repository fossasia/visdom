...
class TestWindowRead(VisdomHTTPTestCase):
    ...
    def test_read_nonexistent_env_all_windows(self):
        resp = self.post_json("/win_data", {"eid": "nonexistent_env", "win": None})
        self.assertEqual(resp.code, 200)
        self.assertEqual(json.loads(resp.body.decode()), {})

    def test_read_nonexistent_env_specific_window_returns_404(self):
        resp = self.post_json("/win_data", {"eid": "nonexistent_env", "win": "any_win"})
        self.assertEqual(resp.code, 404)
        self.assertEqual(resp.reason, "environment not found")

    def test_read_nonexistent_env_unicode_eid_returns_404(self):
        resp = self.post_json(
            "/win_data", {"eid": "nonexistent_env_日本語_🚀", "win": "any_win"}
        )
        self.assertEqual(resp.code, 404)
        self.assertEqual(resp.reason, "environment not found")
...
class TestWindowClose(VisdomHTTPTestCase):
    ...
    def test_close_window_in_nonexistent_env_is_noop(self):
        resp = self.close_window("some_win", eid="nonexistent_env")
        self.assertEqual(resp.code, 200)

    def test_close_all_in_nonexistent_env_is_noop(self):
        resp = self.close_window(None, eid="nonexistent_env")
        self.assertEqual(resp.code, 200)

    def test_close_window_in_nonexistent_unicode_env_is_noop(self):
        resp = self.close_window("some_win", eid="nonexistent_env_日本語_🚀")
        self.assertEqual(resp.code, 200)
