...
class FailingConnector(object):
    """Patched in for ``websocket_connect``: every handshake fails the same way.

    Stands in for the failures that are not a refusal -- a server with no
    ``/vis_socket`` route, a login the cookie does not pass, a TLS or DNS
    failure -- each of which is a handshake that will not start working.
    """
...
class TestWebSocketBackchannel(tornado.testing.AsyncTestCase):
    ...
    @gen_test
    async def test_a_rejected_handshake_runs_socketless(self):
        """A 404 from a server without the route, or a 403 from a login this
        client cannot pass: neither is worth a reconnect either."""
        connector = FailingConnector(HTTPClientError(404))
...
class TestPollingBackchannel(tornado.testing.AsyncTestCase):
    ...
    @gen_test
    async def test_polling_that_never_starts_runs_socketless(self):
        """The same rule on this side: an ``init`` that fails on a client which
        has never heard from the server is not retried for the life of the
        process."""
        attempts = []
        ...
        def respond(url, data):
            ...
            raise requests.exceptions.ConnectionError("server is down")
        ...
        with patch("visdom.async_client.RECONNECT_DELAY", 0):
            ...
            try:
                assert client.use_socket is False
                assert len(attempts) == 1, "polling kept retrying"
            finally:
                await client.shutdown()
