...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2069
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1388
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 681
```
