...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2263
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1507
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 756
```
