...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2599
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1818
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 781
```
