...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2342
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1578
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 764
```
