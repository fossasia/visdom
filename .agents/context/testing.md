...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2512
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1745
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 767
```
