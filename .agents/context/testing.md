...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2236
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1483
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 753
```
