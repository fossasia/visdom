...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2319
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1560
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 759
```
