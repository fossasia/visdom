...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2584
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1797
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 787
```
