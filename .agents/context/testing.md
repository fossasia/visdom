...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2567
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1791
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 776
```
