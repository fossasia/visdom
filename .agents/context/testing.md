...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2589
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1815
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 774
```
