...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2652
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1848
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 804
```
