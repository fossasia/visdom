...
```bash
pytest py/tests --collect-only -q -o addopts="" | tail -1   # 2647
pytest -m unit --collect-only -q -o addopts="" | tail -1    # 1855
pytest -m integration --collect-only -q -o addopts="" | tail -1  # 792
```
