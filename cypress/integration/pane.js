...
basic_examples.forEach((setting) => {
  ...
  describe(`Test Pane Actions on ${type}`, () => {
    ...
    it('Drag & Drop Pane to 2nd Position', () => {
      const targetpos =
        basic_example == 'text_basic'
          ? 263
          : basic_example == 'image_basic'
            ? 276
            : basic_example == 'misc_plot_matplot' ||
                basic_example == 'plot_special_graph'
              ? 10
              : basic_example == 'misc_video_tensor'
                ? 263
                : basic_example == 'misc_audio_basic'
                  ? 350
...
describe('Test Pane Filter', () => {
  it('Open Some Panes', () => {
    ...
    cy.get('.layout .window').should('have.length', 4);
  });

  it('Filter Test 1', () => {
    cy.get('[data-cy="filter"]').type('tag1', { force: true });
    cy.get('.layout .window:visible').should('have.length', 2);
  });

  it('Filter Test 2', () => {
    cy.get('[data-cy="filter"]').clear().type('tag2', { force: true });
    cy.get('.layout .window:visible').should('have.length', 3);
  });

  it('Filter Test 3', () => {
    cy.get('[data-cy="filter"]').clear().type('pane3', { force: true });
    cy.get('.layout .window:visible').should('have.length', 1);
  });
