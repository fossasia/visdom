...
You can subscribe a window to events by adding a function to the event handlers dict for the window id you want to subscribe by calling `viz.register_event_handler(handler, win_id, env_id)` with your handler, the window id, and an optional environment id. Specifying the environment id prevents event handlers from firing across different environments with the same window id. Multiple handlers can be registered to the same window. You can remove event handlers from a window using `viz.clear_event_handlers(win_id, env_id)`. When an event occurs to that window, your callbacks will be called on a dict containing:
