...
Text windows with the same non-empty title in at least two selected environments are also shown. Each remains a separate window with its original content and a `[source]` title prefix, so experiment configurations can be read side by side. The source is the environment name, or its index in the compare legend when an environment name is long.
